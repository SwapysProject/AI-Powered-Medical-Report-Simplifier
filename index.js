...
app.post('/simplify-report', upload.single('image_input'), async (req, res) => {
    ...
    try {
        ...
        if (validation.warnings && validation.warnings.length > 0) {
            finalResponse.warnings = validation.warnings;
        }

        // Add critical value alerts (essential safety feature)
        const criticalTests = parsedData.tests.filter(test => 
            guardrailService.knowledgeBase[test.name.toLowerCase()]?.criticalRanges &&
            (test.value < guardrailService.knowledgeBase[test.name.toLowerCase()].criticalRanges.low ||
             test.value > guardrailService.knowledgeBase[test.name.toLowerCase()].criticalRanges.high)
        );

        if (criticalTests.length > 0) {
            finalResponse.critical_alert = {
                message: "🚨 CRITICAL VALUES DETECTED - Seek immediate medical attention",
                critical_tests: criticalTests.map(t => t.name)
            };
        }
        ...
        finalResponse.status = validation.status === "ok_with_warnings" ? "ok" : validation.status;
